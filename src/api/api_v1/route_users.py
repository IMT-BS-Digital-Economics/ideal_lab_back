...
from src.core.mailing.send_mail import send_verification_mail, send_reset_password_mail
...
async def verified(token: str, db: Session = Depends(get_db)):
    ...
    return {"detail": f"Welcome {db_user.username} ! You are now an approved user !"}
...
async def verify(session_data: SessionData = Depends(verifier), db: Session = Depends(get_db)):
    ...
    return {"detail": "An email has been sent to you"}
...
async def forgot_password(email: UserMail, db: Session = Depends(get_db)):
    ...
    if not db_user:
        return {"detail": "An email has been sent to you in order to reset your password"}
...
async def reset_mail(email: UserMail, session_data: SessionData = Depends(verifier), db: Session = Depends(get_db)):
    ...
    return {"detail": f"You have changed your email to {result.email}"}
